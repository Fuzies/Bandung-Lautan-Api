...
func _on_HurtBoxInterract_area_entered(area):
	var questcheck = global.get_questcompleted()
	for i in questcheck.size():
		if questcheck[i] == object_name:
			index = 2
	...
	if quest_complete and index != 2:
		...
		if quest_reward != "" and !reward_dropped:
			...
			global.questcompleted.append(object_name)
			global.saveprogress()
